...
def get_user():
    ...
    if req_username not in game_state.usernames:
        ...
        game_state.current_score_map[req_username] = { "total": 0 }
    else:
...
def get_user_with_current_turn():
    ...
    user = game_state.user_with_turn
    game_state.game_status_message = f"{user} has the current turn. Score: {game_state.current_score_map[user]}. Dice values: {game_state.dice_values}."
    emit('broadcast_game_state', get_game_state(), broadcast=True)
...
def handle_start_game():
    ...
    user = game_state.user_with_turn
    game_state.game_status_message = f"{user} has the current turn. Score: {game_state.current_score_map[user]}. Dice values: {game_state.dice_values}."
    game_state.transcript.append(game_state.user_with_turn + " has the current turn.")
    emit('broadcast_game_state', get_game_state(), broadcast=True)
...
def handle_end_turn(user, player_score):
    ...
    next_user = game_state.usernames[game_state.turn_idx]
    game_state.user_with_turn = next_user
    game_state.game_status_message = f"{next_user} has the current turn. Score: {game_state.current_score_map[next_user]}. Dice values: {game_state.dice_values}."
    game_state.transcript.append(f"Turn has ended for {user}. {user}'s score: {player_score}. Now " + next_user + " has the current turn.")
    emit('broadcast_game_state', get_game_state(), broadcast=True)
...
def handle_dice_values(dice_values):
    ...
    user = game_state.user_with_turn
    game_state.game_status_message = f"{user} has the current turn. Score: {game_state.current_score_map[user]}. Dice values: {dice_values}."
    game_state.transcript.append(f"Dice values for {user}: {dice_values}.")
    emit('broadcast_game_state', get_game_state(), broadcast=True)
